	...
	<div id="main-wrapper">
		<div id="main" class="5grid-layout">
			<div class="row">
				...
				<h2>Records by: <b><%=request.getParameter("username")%></b></h2>
...
<%
    ...
    Query query = new Query("AudioRecord").addFilter("file_owner", FilterOperator.EQUAL, request.getParameter("username")).addFilter("file_visibility", FilterOperator.EQUAL, "public");
    List<Entity> records;
...
%>
		<li>
			<article class="is-post-summary">
				<h3><a href="/view.jsp?item=<%= KeyFactory.keyToString(record.getKey()) %>"><%=record.getProperty("file_title")%></a></h3>
					<ul class="meta">
					...
					%>
					<ul class="meta">
						<li class="tag"><%= tag.getProperty("record_tag")%></li>
					</ul>
					  <% } } %>
			</article>
		</li>
...
<% if (isPageFilled == true) { %>			
	<div style="text-align: center">
		...
		%>
		<a href="http://localhost:8888/viewprofile.jsp?username=<%=request.getParameter("username")%>&offset=<%= offset %>" class="button button-alt">Show older records</a>
	</div>
<% } %>
						</section>
					</div>							
