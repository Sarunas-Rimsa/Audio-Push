	...
	<div id="main-wrapper">
		<div id="main" class="5grid-layout">
			<div class="row">
				...
				<h2>Records tagged with <b>#<%=request.getParameter("view")%></b></h2>
...
<%
    ...
    Query query = new Query("Tag").addFilter("record_tag", FilterOperator.EQUAL, request.getParameter("view"));
    List<Entity> taggedRecords;
    
    boolean isPageFilled = false;
    
    if(request.getParameter("offset") != null) {
    	int offset = Integer.parseInt(request.getParameter("offset"));
    	taggedRecords = datastore.prepare(query).asList(FetchOptions.Builder.withOffset(offset).limit(15));
    	isPageFilled = true;
    }
    else {
    	taggedRecords = datastore.prepare(query).asList(FetchOptions.Builder.withLimit(15));
   	}
    
	int recordCounter = 0;
    for (Entity taggedRecord : taggedRecords) {
    
    	Entity record = datastore.get(KeyFactory.stringToKey(String.valueOf(taggedRecord.getProperty("record_key"))));
    
	    Key guestbookKey = KeyFactory.createKey("Guestbook", KeyFactory.keyToString(record.getKey()));
	    Query que = new Query("Greeting", guestbookKey);
	    List<Entity> greetings = datastore.prepare(que).asList(FetchOptions.Builder.withLimit(50));
...
<% if (isPageFilled == true) { %>			
	<div style="text-align: center">
		...
		%>
		<a href="http://localhost:8888/tag.jsp?view=<%=request.getParameter("view")%>&offset=<%= offset %>" class="button button-alt">Show older records</a>
	</div>
<% } %>
						</section>
					</div>							
